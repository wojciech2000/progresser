  ...
  const displayData = dataType => {
    if (!currentData && dataType === "span") {
      ...
      return arrayDatas.map((data, id) => {
        if (data[0] !== "image" && dataType === "span")
          return (
            <span
              key={id}
              style={
                !image ? { width: "100%", textAlign: "center" } : undefined
              }>
              {data[0]}: {data[1]}cm
            </span>
          );
        else if (data[0] === "image" && dataType === "image")
          return (
            <img
              key={id}
  ...
  return (
    <motion.div
      ...
      <div className='current__section'>
        {displayData("image")}
        <div className='current__data'>{displayData("span")}</div>
      </div>
    </motion.div>
  );
}
